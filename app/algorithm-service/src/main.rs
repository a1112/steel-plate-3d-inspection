...
fn source_adapter_loop(state: Arc<State>) {
    while !state.shutdown.load(Ordering::Acquire) {
        scan_sources(&state);
        if let (Some(runtime), Some(adapter)) = (state.mysql_runtime.as_ref(), state.bkv.as_ref()) {
            if let Err(error) =
...
fn status_json(state: &State) -> String {
    ...
    json!({"schema":"steel.algorithm-service.status.v1","service":"steel-algorithm-service","ready":true,"resultRoot":state.publisher.root(),"inputRoots":state.input_roots,"processor":state.processor,"scanInProgress":state.scan_in_progress.load(Ordering::Acquire),"counters":{"scanned":counters.scanned,"published":counters.published,"skipped":counters.skipped,"failed":counters.failed,"lastError":counters.last_error,"lastPublishedAt":counters.last_published_at}}).to_string()
}
