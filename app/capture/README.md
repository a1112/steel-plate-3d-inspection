...
- `steel_runtime_supervisor`: Windows SCM host for the capture, trigger, and Rust runtime group.

The Supervisor writes its atomic runtime state to `StateRoot\service\supervisor-status.json`. A restart budget exhausted within the 600-second window remains persisted across Supervisor exit and is cleared only after 30 seconds of stable whole-group runtime, allowing the Rust service to create and later resolve a durable system-health alarm.
...
For the current eight-camera hardware profile, prefer the stack starter:

```powershell
scripts/start-capture-stack.ps1 -StopExisting
```

It starts the provider on port `4317`, uses the configured storage root, applies `current-8-time-trigger`, and preserves the camera's current built-in/device parameters. No Qt runtime is built or started. Pass `-ApplyPreset` only when intentionally forcing the generic line preset from the API.
...
When Rust runs with `STEEL_CAPTURE_PROVIDER=headless-cpp`, it can start this executable automatically. `external-api` connects to an already running provider, and `simulated` runs without cameras.
...
- `POST /api/config/profile/apply` with `{"name":"default","autoConnect":true,"expectedCameras":8}`
...
- `POST /api/cameras/connect-all` with `{"expectedCameras":8,"devType":-1}`
...
- `POST /api/capture/continuous-test` with optional `{"expectedCameras":8,"rounds":3,"lines":1280,"intervalMs":500,"dataMode":3,"outputDir":"continuous-test","connectFirst":true}`
...
- `POST /api/calibration/active` with `{"name":"current-8-time-trigger","path":"config/calibrations/current-8-time-trigger/<version>/ArrayCalibration.corrected.xml"}`
...
The formal Rust `/api/calibration/apply-all` route is deliberately narrower than the provider's diagnostic contract. It requires exactly eight unique target IPs and exactly eight matching `cameraCalibrations`. Every mapping must contain a non-empty, set-unique `expectedSn` and a distinct normalized SDK calibration path, and the request must set `expectedCameras:8`, `atomic:true`, `rollbackOnFailure:true`, `requireAllMapped:true`, and `stopStreams:true`. A real apply also requires a caller-owned stable `operationId`. Rust rejects `allowBestEffortDeviceRollback:true` and rejects `saveCameraParams:true`; camera-parameter snapshot saving is a separate maintenance operation and is not part of the formal atomic calibration transaction.

```json
{
  "operationId": "calibration-apply-20260712-001",
  "name": "current-8-time-trigger",
  "path": "config/calibrations/current-8-time-trigger/<version>/ArrayCalibration.corrected.xml",
  "ips": [
    "192.168.101.100",
    "192.168.102.100",
    "192.168.103.100",
  ...
  "cameraCalibrations": [
    ...
    },
    {
      ...
      "artifactType": "camera-sdk",
      "path": "config/calibrations/<version>/camera-107.xml",
    ...
    },
    {
      ...
      "expectedSn": "YF-0269",
      "artifactType": "camera-sdk",
      "path": "config/calibrations/<version>/camera-108.xml",
...
Relative output paths are stored under `CAPTURE_STORAGE_ROOT`, or `H:\` by default when drive `H:` exists. Per-camera production roots can be configured with `/api/storage/camera-roots`; the current eight-camera default maps the known camera IPs to `H:\camera1` through `H:\camera8`. Realtime stream start uses the LVM async path and keeps only one stream active in the provider process. Blocking depth capture rejects requests while a realtime stream is running for the same camera. Continuous-test capture runs one worker thread per camera in each round and releases the workers through a shared start gate; the response includes `parallel`, `syncMode`, `workerCount`, and per-frame worker timestamps. Frame files are grouped under each camera directory by peer artifact folders: `depth/`, `intensity/`, and `metadata/`; `sdk-derived/` is saved only when the request explicitly sends `saveSdkDerived:true` or `save_sdk_derived:true`.
...
The current eight-camera production baseline is continuous line capture with the existing device configuration, vendor time-trigger source, 1000 lines, and depth + intensity output: `controlMode=0`, `triggerInputType=4`, `lines=1000`, `dataMode=3`. The packaged `current-8-time-trigger` profile uses `loadCameraParams:false` and `changeStorage:false` so startup does not overwrite the verified device-side setup. Operator tests and maintenance are exposed by Tauri through Rust.
