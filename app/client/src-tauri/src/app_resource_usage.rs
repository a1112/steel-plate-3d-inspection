...
#[tauri::command]
pub(crate) fn app_resource_usage() -> Result<AppResourceUsage, String> {
...
mod tests {
    ...
    #[test]
    fn current_process_is_included_in_the_snapshot() {
        let usage = collect_app_resource_usage().expect("current Tauri process should be visible");

        assert_eq!(usage.source, "tauri");
        assert_eq!(usage.precision, "full");
