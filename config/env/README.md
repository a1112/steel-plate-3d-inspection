...
- The unified service reads secrets from the absolute path in `STEEL_RUNTIME_SECRET_ENV_FILE`. Its installer accepts the trigger/database/bootstrap keys and the `STEEL_BKV_*` algorithm-adapter keys in that file, rejects duplicate/unknown keys, and applies SYSTEM/Administrators-only ACLs. The Supervisor passes BKV keys only to `steel-algorithm-service`; the business and image services never receive them. Keep storage roots, ports, allowlists, and production mode in the generated non-secret `runtime-service.env` so a secret rotation cannot override safety policy.
