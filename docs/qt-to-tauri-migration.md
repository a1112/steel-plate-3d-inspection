...
The runtime consists of Tauri, Rust, the trigger gateway, and the headless C++ capture service. Qt source, provider compatibility, build scripts, startup switches, and package branches have been removed.

Migration completeness is not the same as production release readiness. Production now configures offline WebView2 and a Microsoft-signed VC++ prerequisite gate, but actual signed MSI/NSIS output, the approved algorithm qualification report, Supervisor/SCM target-machine evidence, current eight-camera acceptance, disaster recovery and site signoff remain separate gates in [the release, deployment, and operations SOP](release-deployment-and-operations.md).
...
- Capture overview, eight-camera status, connect/disconnect, exposure/gain write, production steel-info/steel-in/capture/steel-out controls, and trigger gateway status/modes/manual controls. All operator calls terminate at Rust; the browser never calls the capture or trigger ports directly.
...
- Service/database/task-worker/capture/calibration-reconciliation/storage/trigger/algorithm/production-policy health visibility in the Tauri header, including the failed readiness check, unresolved operation IDs, and stable reason code.
- Rust exposes separate `/api/health/live`, `/api/health/ready`, `/api/health/ready/details`, and `/api/health/details` endpoints. Readiness performs a real database ping, checks the persistent-task worker, requires the calibration-reconciliation ledger to be clear, probes the configured non-simulated capture provider and its bounded writer queue, checks the standalone trigger gateway, and rejects missing algorithm qualification or an invalid production policy without leaking origins, paths, IP addresses, credentials, or raw dependency bodies.
- Bar-surface reconstruction and target-gated automatic array calibration: enqueue a durable task, capture one real round from exactly eight unique cameras, require eight complete frames and eight metadata commits, detect a round calibration target by identity, point count, diameter, angular coverage, robust inlier ratio, and residual thresholds, and only generate/activate corrected reconstruction XML when correction quality also passes. No path writes camera devices.
- Admin configuration, users/roles, audit, diagnostics, database maintenance, and record retention.
- Startup and packaging use the headless C++ provider and Tauri client exclusively.
...
- Admin record list, detail, CSV export, delete, and retention now use `production_inspection`, `material_session`, `production_defect`, and `capture_file`. Deletion uses a persistent allowed-root/hash-frozen cleanup manifest, removes physical production artifacts first with retryable per-file progress, then transactionally removes inspection-owned indexes; the material session and trigger/secondary history remain retained.
...
- Tauri now exposes provider profile read/create/save/provider-local-path import/apply, eight-camera connect-all, complete SDK parameter readback, camera parameter-file save/load/recovery, global and per-camera storage roots, continuous-test parameters/run/results, and writer-queue metrics. The structured profile editor synchronizes online camera IP/model/SN and current SDK exposure/gain/trigger-frequency readback. Device-persistent writes remain disabled by default and risky operations require exact typed confirmation.
...
- Eight-camera SDK calibration deployment now requires eight unique IPs, eight expected serial numbers, and eight per-camera SDK calibration files. It rejects array-reconstruction XML as a camera SDK artifact, requires a successful unchanged dry-run before apply, defaults to `saveToDevice=false`, supports atomic rollback, exposes per-camera preflight/apply/persist/rollback codes, and returns an explicit rollback token. Array XML remains reconstruction-only.
...
| --- | --- | --- |
| Overview, current material, eight-camera cards, realtime SDK/readback status, health, queue state, logs, and refresh | Tauri UI through Rust read APIs | Migrated; logs merge bounded provider/system/operator evidence and readiness includes calibration reconciliation |
...
| Structured per-camera profile fields: enable, IP, model, SN, parameter source/file, storage root, exposure, gain, and trigger frequency | Tauri structured editor; online status/readback comes through Rust | Migrated, including online IP/model/SN/SDK-value synchronization and editable `H:/camera1..camera8` mappings |
...
| Automatic calibration capture/fit, target detection, quality-gated correction, `fit_report` import, before/after review, active refresh, and reconstruction-version activation | Tauri bar-surface UI plus Rust durable tasks/algorithm services | Migrated; fit requires complete frames and metadata from eight unique cameras, and activation never writes camera devices |
...
| Eight-camera per-device calibration preflight/apply/result/rollback and optional parameter/device persistence | Tauri controlled diagnostic UI through Rust's persistent operation ledger; C++ validates artifact identity, SN binding, staged previous files, SHA-256, generation, and rollback | Every mapping needs a known-good rollback file, array XML cannot be reused as a per-camera file, and ambiguous operations remain fenced until a parent-bound rollback is verified |
...
Qt has been removed from source, runtime, packaging, and provider configuration. The items below are production validation gates for the migrated architecture.

Before the hardware gates below, release management must also close the non-migration gates: clean/tagged Release build; approved `status=pass` algorithm report and per-run traceability; signed/timestamped desktop and background PE inventory; offline WebView2/VC++/SDK prerequisites; and two-stage installation of the background service plus desktop client. A green migration/static contract cannot substitute for those checks.

### P0 - Real eight-camera hardware acceptance

- Rerun the complete workflow against eight real LVM/NVT cameras. The current green acceptance uses the simulated provider and proves process/API/storage behavior, not vendor SDK or device persistence behavior.
...
- The remaining work is real-device proof: exercise restart during apply and rollback, verify SN/generation/hash rejection performs zero writes, verify `saveToDevice=true` restores known-good files on all eight cameras, and record validation-frame plus maintenance evidence after reconciliation.

### Non-Qt product backlog

- Production-chain dependency semantics are complete: persisted `chainId`, `dependsOnTaskId`, `dependencyPolicy`, and `blockedReason` fields enforce a single chain per material/session; safety-critical work defaults to `require-success`, recursively blocks after prerequisite failure/cancellation/interruption, and is requeued with descendants when the failed parent is retried. `always-run` is restricted to explicitly safe `trigger-event` cleanup.
...
- Defect-specific camera/sequence/ROI geometry and review artifacts are now emitted as `steel.surface.defect.artifacts.v1`: intensity/depth ROI images, a local point cloud, and independent length/width profiles are stored under the algorithm run, persisted through `geometry_json`, exposed in production snapshots, and loaded by defect ID. Older records still show the selected record's full capture/mesh only as an explicitly labelled non-local fallback. Real eight-camera defect samples must still validate ROI mapping and dimensional accuracy.
- Rust now exposes capture root, algorithm output root, config identity/hash and thresholds as `steel.algorithm-runtime-config.v1` desired/active/readback; the installer derives the active roots from reviewed site storage. Remaining work is target-machine readback and changeover validation.
- Formal inspection reports now have a backend-owned content-addressed JSON snapshot, immutable archive, idempotent issuance and per-inspection history query. The Tauri report page can issue a single production record and displays archive history state; printable evidence-image/PDF layout remains product-template work.
...
- Qt compatibility mode no longer exists.
- Array reconstruction XML is never a per-camera SDK calibration artifact. An eight-camera apply requires eight explicit mappings, unique IP/SN identity, an unchanged successful dry-run, and typed confirmation; device persistence has an additional confirmation and defaults off.
...
- Older real-hardware evidence covers a previous six-camera layout; the current eight-camera connection verification does not replace a full capture/calibration stability run.
