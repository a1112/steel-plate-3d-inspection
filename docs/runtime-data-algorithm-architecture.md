...
- `bkv-online-mysql`：**已暂时弃用并隔离**；仅独立适配器可在受控迁移/兼容验证中读取现场 MySQL 与 `CamImageSource*` 共享目录，不能激活为常规现场配置；
...
弃用的 BKV online 兼容处理器由独立进程持有原始数据源凭据；业务服务只读取统一结果库，不构造 MySQL/共享目录连接。兼容处理器不复制几百兆原始图像，而是按需生成缓存，并把较小的业务记录与缺陷明细转存为原子 JSON 快照，供受控迁移追溯。

## 产物布局

弃用的 BKV online 兼容配置输出到：
