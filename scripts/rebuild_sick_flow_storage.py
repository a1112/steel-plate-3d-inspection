#!/usr/bin/env python3
"""Offline one-way rebuild into the strict flow-first SICK storage layout.

This is deliberately not a compatibility migration.  It moves immutable raw
artifacts from <disk>/<camera>/<flow> into
<disk>/<flow>/capture/<camera>, discards every derived/cache artifact, rebuilds
durable frame events and playback indexes, and rewrites PostgreSQL file paths.
"""
...
def build_plan(camera_roots: dict[str, Path]) -> list[MoveAction]:
    actions: list[MoveAction] = []
    for camera_id, target_root in camera_roots.items():
        legacy_root = target_root / camera_id
        if not legacy_root.is_dir():
            continue
        for source_flow in legacy_root.iterdir():
            if not source_flow.is_dir() or not source_flow.name.isdecimal():
                continue
            flow_id = str(int(source_flow.name))
            target = capture_root(target_root, flow_id, camera_id)
            for name in (*RAW_DIRECTORIES, *RAW_FILES):
                actions.append(
                    MoveAction(source_flow / name, target / name, camera_id, flow_id)
                )
    return actions
...
def validate_plan(actions: Sequence[MoveAction], *, check_targets: bool = True) -> None:
    ...
    for action in actions:
        identity = str(action.target.absolute()).lower()
        if identity in targets:
            raise FileExistsError(action.target)
        targets.add(identity)
        grouped[(action.source.parent, action.target.parent)].add(action.source.name)
    if not check_targets:
        return
    for (source_flow, target_capture), required in grouped.items():
        available = {child.name for child in source_flow.iterdir()}
        missing = required - available
        if missing:
            raise FileNotFoundError(
                f"incomplete immutable raw flow {source_flow}: {sorted(missing)}"
            )
        if target_capture.exists():
            raise FileExistsError(target_capture)
...
def _retire_legacy_camera_roots(camera_roots: dict[str, Path]) -> dict[str, Any]:
    """Remove the unsupported camera-first namespace from all data disks."""
    stamp = dt.datetime.now().strftime("%Y%m%d-%H%M%S")
    retired: list[dict[str, str]] = []
    for camera_id, root in camera_roots.items():
        source = root / camera_id
        if not source.exists():
            continue
        _assert_child(source, root)
        destination = root / "rebuild" / f"obsolete-camera-v1-{stamp}" / camera_id
        _assert_child(destination, root)
        destination.parent.mkdir(parents=True, exist_ok=True)
...
def _database_sql(storage_root: Path, camera_roots: dict[str, Path]) -> str:
    ...
    commands.append(
        ...
        f"depth_path=roots.root || image.flow_no::text || '{os.sep}capture{os.sep}' || image.camera_id || '{os.sep}3d{os.sep}' || (image.camera_sequence_no-1)::text || '.npz', "
        f"intensity_path=roots.root || image.flow_no::text || '{os.sep}capture{os.sep}' || image.camera_id || '{os.sep}2d{os.sep}' || (image.camera_sequence_no-1)::text || '.png', "
        f"metadata_path=roots.root || image.flow_no::text || '{os.sep}capture{os.sep}' || image.camera_id || '{os.sep}json{os.sep}' || (image.camera_sequence_no-1)::text || '.json' "
    ...
    commands.append(
        ...
        f"path=roots.root || flow.flow_no::text || '{os.sep}capture{os.sep}' || capture.camera_id || '{os.sep}' || "
        ...
        f"metadata_path=roots.root || flow.flow_no::text || '{os.sep}capture{os.sep}' || capture.camera_id || '{os.sep}json{os.sep}' || (capture.sequence_no-1)::text || '.json' "
...
def _rewrite_database(env_path: Path, sql: str) -> None:
    ...
    if not database_url:
        raise ValueError("STEEL_DATABASE_URL is required for database rebuild")
    psql = os.environ.get("PSQL_EXECUTABLE", r"D:\PostgreSQL\17\server\bin\psql.exe")
    subprocess.run(
        [psql, "-X", "-w", "-v", "ON_ERROR_STOP=1", "-d", database_url, "-c", sql],
        check=True,
        env=os.environ.copy(),
    )
...
def rebuild(profile_path: Path, env_path: Path | None, *, execute: bool) -> dict[str, Any]:
    ...
    actions = build_plan(camera_roots)
    ...
    result: dict[str, Any] = {
        "schema": "steel.flow-storage-rebuild.v2",
    ...
    journal = storage_root / "rebuild" / f"flow-storage-v2-{dt.datetime.now():%Y%m%d-%H%M%S}.json"
    ...
    try:
        ...
        for action in actions:
            ...
            touched_sources.add(action.source.parent)
        for source in sorted(
            touched_sources, key=lambda path: (len(path.parts), str(path)), reverse=True
        ):
            if not source.is_dir():
                continue
            for legacy_derived in ("alignment.json", "measurement.json"):
                (source / legacy_derived).unlink(missing_ok=True)
            if not any(source.iterdir()):
                source.rmdir()
        result["legacyCameraRetirement"] = _retire_legacy_camera_roots(camera_roots)
